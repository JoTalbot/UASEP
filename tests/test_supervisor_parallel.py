...
def test_parallel_runtime_persists_completion(tmp_path: Path):
    ...
    tasks = [
        Task("A", "first", write_set=["a.py"]),
        Task("B", "second", write_set=["b.py"]),
    ]
...
def test_parallel_runtime_does_not_run_write_conflicts_together(tmp_path: Path):
    ...
    tasks = [
        Task("A", "first", write_set=["shared.py"]),
        Task("B", "second", write_set=["shared.py"]),
    ]
    ...
    assert tasks[1].status == TaskStatus.READY or tasks[1].status == TaskStatus.BACKLOG
